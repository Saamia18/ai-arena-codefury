...
export function HistoryPage() {
  ...
  const entries = filter === 'All' ? mockHistoryEntries : mockHistoryEntries.filter((entry) => entry.status === filter)
  const refreshHistory = () => { setIsLoading(true); window.setTimeout(() => setIsLoading(false), 450) }
  return <section className="history-page page-container"><div className="history-topline"><span>PERSONAL ARENA ARCHIVE</span><span>{mockHistoryEntries.length} RECORDED DECISIONS</span></div><div className="history-heading"><div><p className="eyebrow"><i /> Your decision trail</p><h1>My <em>Passports.</em></h1></div><p>Every saved Quest and Arena verdict—ready to revisit, compare, and put to work.</p></div><div className="history-summary"><div><strong>{mockHistoryEntries.length}</strong><span>Quest profiles</span></div><div><strong>{mockHistoryEntries[0].trustScore}</strong><span>Latest Trust Score</span></div><div><strong>{mockHistoryEntries.filter((item) => item.status === 'Saved').length}</strong><span>Saved passports</span></div></div><div className="history-toolbar"><div>{(['All', 'Saved', 'Completed'] as const).map((option) => <button type="button" key={option} className={filter === option ? 'is-active' : ''} onClick={() => setFilter(option)}>{option === 'All' ? 'All activity' : option}</button>)}</div><button className="history-refresh" type="button" onClick={refreshHistory} disabled={isLoading}>{isLoading ? 'Refreshing…' : 'Refresh archive'} ↻</button></div>{isLoading ? <div className="history-loading"><span /><strong>Refreshing your Arena archive…</strong></div> : entries.length === 0 ? <div className="history-empty"><span>◌</span><h2>No {filter.toLowerCase()} records yet.</h2><p>Start a new Quest to create a fresh AI decision trail.</p><Link className="button" to="/quest">Begin AI Quest <span>→</span></Link></div> : <div className="history-list">{entries.map((entry) => { const createdAt = new Date(entry.createdAt); const priority = [...dnaDimensions].sort((a, b) => entry.profile[b.key] - entry.profile[a.key])[0]; return <article className="history-card" key={entry.id}><Link className="history-card__main" to={`/passport/${entry.id}`}><div className="history-card__status"><span>{entry.status}</span><time dateTime={entry.createdAt}>{createdAt.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</time></div><div className="history-card__model"><span>{entry.winner.charAt(0)}</span><div><h2>{entry.winner}</h2><p>{entry.provider}</p></div></div><div className="history-card__scores"><div><span>TRUST SCORE</span><strong>{entry.trustScore}</strong></div><div><span>MATCH SCORE</span><strong>{entry.matchScore}</strong></div></div><div className="history-card__profile"><span>TOP AI DNA SIGNAL</span><b>{priority.label}</b><small>{Math.round(entry.profile[priority.key] * 100)}%</small></div><span className="history-card__arrow">→</span></Link><Link className="history-card__passport" to={`/passport/${entry.id}`}>View Passport</Link></article> })}</div>}<p className="history-future-note">Mock archive view · ready for future <code>GET /api/quest/history</code> and <code>GET /api/results</code> integration.</p></section>
}
