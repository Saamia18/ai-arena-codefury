...
export function ResultPage() {
  ...
  const matchScore = typeof state.matchScore === 'number' ? state.matchScore : null
  ...
  return <section className="result-page page-container"><div className="result-topline"><span>RECOMMENDED FOR YOU</span><span>{usesMockResult ? 'DEMO RESULT' : 'BACKEND-VERIFIED RESULT'}</span></div><div className="result-heading"><div><p className="eyebrow"><i /> Step 04 / Trust Score</p><h1>Your best fit,<br /><em>made clear.</em></h1></div><p>Match Score shows fit with your AI DNA; Trust Score shows confidence in the recommendation.</p></div><div className="result-grid"><article className="winner-card winner-card--recommended"><span className="winner-card__label">RECOMMENDED FOR YOU</span><div className="winner-card__model"><span>{winner.name.charAt(0)}</span><div><h2>{winner.name}</h2><p>{winner.provider}</p></div></div><div className="winner-card__scores"><div><span>MATCH SCORE</span><strong>{matchScore ?? '—'}</strong></div><div><span>TRUST SCORE</span><strong>{displayedTrustScore?.overallTrustScore}</strong></div></div></article><article className="explanation-card"><span className="winner-card__label">WHY IT FITS YOUR AI DNA</span><p>{displayedExplanation}</p><div className="recommendation-signals">{dnaDimensions.map((dimension) => <span key={dimension.key}><b>{dimension.label}</b><small>Your priority {Math.round(profile[dimension.key] * 100)}% · Model strength {winner[dimension.key]}</small></span>)}</div></article></div><section className="trust-section"><div><p className="eyebrow">Trust breakdown</p><h2>Confidence, made <em>visible.</em></h2></div><div className="trust-bars">{Object.entries(displayedTrustScore?.breakdown ?? {}).map(([label, value]) => <div className="trust-bar" key={label}><div><strong>{label === 'easeOfUse' ? 'Ease of use' : label}</strong><span>{value}</span></div><div><span style={{ width: `${value}%` }} /></div></div>)}</div></section><section className="runner-ups"><div><p className="eyebrow">Why not the others?</p><h2>Every contender gets an explanation.</h2></div><div>{displayedReasons.map((reason) => <details key={reason.modelId}><summary>{runnerUps.find((model) => model.id === reason.modelId)?.name ?? reason.modelId}<span>+</span></summary><p>{reason.reason}</p></details>)}</div></section><div className="result-actions"><div>{saveError && <p className="result-error" role="alert">{saveError}</p>}</div><button className="button" type="button" onClick={saveResult} disabled={isSaving}>{isSaving ? 'Saving result…' : usesMockResult ? 'View Model Passport' : 'Save Model Passport'} <span>→</span></button></div></section>
}
